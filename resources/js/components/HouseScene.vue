...
async function load(){const request=++version;error.value='';try{const gltf=await new GLTFLoader().loadAsync(`/models/${props.floor}.glb`);if(request!==version){dispose(gltf.scene);return}if(model){scene.remove(model);dispose(model)}model=gltf.scene;scene.add(model);updatePins()}catch{error.value='The 3D model could not be loaded. Try refreshing the page.'}}
...
watch(()=>props.floor,()=>{if(scene)load()});watch(()=>[props.selected,props.lights,props.aliases],()=>{if(scene)updatePins()},{deep:true});
