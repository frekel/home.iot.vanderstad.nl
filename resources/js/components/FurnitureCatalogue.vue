...
// Same plan reflections as HouseScene; SVG's downward Y inverts the plan Y.
...
const fields:{key:Dimension;label:string;min:number;max:number}[]=[
 {key:'width',label:'Breedte',min:5,max:1100},{key:'depth',label:'Diepte',min:1,max:550},
 {key:'height',label:'Hoogte',min:1,max:400},{key:'base_z',label:'Onderkant vanaf vloer',min:0,max:300},
];
const drafts=ref<Record<string,Record<Dimension,string>>>({});
const editRevision=ref<number>();
const saving=ref(false),saveError=ref('');
const busy=computed(()=>saving.value||['queued','building'].includes(furnitureState.value?.status??''));
const changed=computed(()=>(furnitureState.value?.items??[]).filter(i=>drafts.value[reference(i)]));
const buildMessage=computed(()=>saving.value?'Wijzigingen opslaan…':({queued:'Opgeslagen · wachten op opbouw…',building:'Plattegrond wordt opgebouwd. Het vorige model blijft zichtbaar.',ready:'Plattegrond bijgewerkt.',failed:'Opbouwen mislukt. Je maten zijn opgeslagen; het vorige model blijft zichtbaar.'}[furnitureState.value?.status??'ready']));
function value(i:Item,key:Dimension){return drafts.value[reference(i)]?.[key]??String(Math.round(i[key]*100000)/1000)}
function edit(i:Item,key:Dimension,event:Event){
 if(editRevision.value===undefined)editRevision.value=furnitureState.value?.revision;
 const id=reference(i);
 drafts.value[id]??=Object.fromEntries(fields.map(f=>[f.key,value(i,f.key)])) as Record<Dimension,string>;
 drafts.value[id][key]=(event.target as HTMLInputElement).value;
 saveError.value='';
}
function discard(){drafts.value={};editRevision.value=undefined;saveError.value='';void refreshFurniture().catch(()=>{})}
async function save(){
 if(!furnitureState.value||busy.value)return;
 const updates=[];
 for(const item of changed.value){
  const dimensions={} as Record<Dimension,number>;
  for(const field of fields){
   const raw=value(item,field.key),number=Number(raw);
   if(raw.trim()===''||!Number.isFinite(number)||number<field.min||number>field.max){saveError.value=`${reference(item)}: ${field.label} moet tussen ${field.min} en ${field.max} cm liggen.`;return}
   dimensions[field.key]=number;
  }
  updates.push({floor:item.floor,id:item.id,...dimensions});
 }
 saving.value=true;saveError.value='';
 try{
  const csrf=document.querySelector<HTMLMetaElement>('meta[name="csrf-token"]')?.content??'';
  const response=await fetch('/dashboard/furniture',{method:'PUT',headers:{Accept:'application/json','Content-Type':'application/json','X-CSRF-TOKEN':csrf},body:JSON.stringify({revision:editRevision.value??furnitureState.value.revision,items:updates}),signal:AbortSignal.timeout(20000)});
  const data=await response.json();
  if(!response.ok)throw new Error(data.errors?Object.values(data.errors).flat().join(' '):data.message??'Opslaan mislukt.');
  furnitureState.value=data;drafts.value={};editRevision.value=undefined;
 }catch(error){saveError.value=error instanceof Error?error.message:'Opslaan mislukt. Controleer de verbinding en probeer opnieuw.'}
...
<div class="modal-backdrop furniture-backdrop" @click.self="!changed.length && $emit('close')"><section class="furniture-catalogue" role="dialog" aria-modal="true" aria-labelledby="furniture-title">
 ...
 <h2 id="furniture-title">Meubels wijzigen</h2><p>Selecteer een meubel, pas de maten in centimeters aan en sla alle wijzigingen samen op.</p>
 <div class="furniture-savebar">
 <button class="primary" :disabled="busy || !furnitureState || (!changed.length && furnitureState.status!=='failed')" @click="save">{{saving?'Opslaan…':furnitureState?.status==='failed'&&!changed.length?'Opnieuw opbouwen':'Opslaan en plattegrond bijwerken'}}{{changed.length?' ('+changed.length+')':''}}</button>
 <button v-if="changed.length" :disabled="busy" @click="discard">Wijzigingen annuleren</button>
 <p role="status" v-if="furnitureState && (busy || furnitureState.revision>0)">{{buildMessage}}</p>
 ...
 <svg :viewBox="`${-current.length/2-0.7} -3.6 ${current.length+1.4} 7.2`" role="img" :aria-label="`Meubelkaart ${floorNames[floor]}`">
  ...
  <g v-for="m in markers" :key="m.item.id" role="button" tabindex="0" :aria-label="`${reference(m.item)} ${label(m.item)}`" @click="choose(m.item)" @keydown.enter.prevent="choose(m.item)" @keydown.space.prevent="choose(m.item)" style="cursor:pointer">
   <line :x1="position(m.item.x,m.item.y)[0]" :y1="position(m.item.x,m.item.y)[1]" :x2="m.x" :y2="m.y" stroke="#dfaf70" stroke-width=".018"/>
   <circle :cx="m.x" :cy="m.y" r=".19" :fill="isSelected(m.item)?'#ff8a18':'#131b24'" stroke="#ffaf59" stroke-width=".017"/>
   <text :x="m.x" :y="m.y+.047" text-anchor="middle" font-size=".135" :fill="isSelected(m.item)?'#101419':'#fff'">{{reference(m.item).split('-')[1]}}</text>
  </g>
 </svg>
 <p>Kaartnummers beginnen met <strong>{{prefixes[floor]}}-</strong>. De kaart volgt de spiegeling van de 3D-plattegrond.</p>
 <div class="furniture-selection" aria-live="polite"><template v-if="chosen"><strong>{{reference(chosen)}} · {{label(chosen)}}</strong><p>{{room(chosen)}} · {{dimensions(chosen)}}</p><p v-if="chosen.group"><strong>{{chosen.group}} · {{chosen.group_name}}</strong> — {{items.filter(i=>i.group===chosen?.group).map(reference).join(', ')}}. Geef dit groepsnummer door om alles samen te wijzigen.</p><fieldset class="furniture-fields" :disabled="busy || !furnitureState">
 <label v-for="field in fields" :key="field.key">{{field.label}} (cm)<input type="number" step="0.1" :min="field.min" :max="field.max" :value="value(chosen,field.key)" @input="edit(chosen,field.key,$event)"/></label>
 </fieldset><p>Maten wijzigen rond het middelpunt van het meubel. Hoge meubels kunnen in de plattegrond op muurhoogte worden afgesneden.</p><p>Geef bijvoorbeeld door: “{{reference(chosen)}} 20 cm naar rechts op de meubelkaart.”</p></template><span v-else>Selecteer een meubel voor het volledige nummer.</span></div>
